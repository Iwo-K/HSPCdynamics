...
def paga(
    ...
    connectionstyle
        directed edge style argument, passed to networkx.draw_networkx_edges.
        Default ('arc3') specifies straigth arrows, curved arrows can be
        specified with e.g. 'arc3, rad=0.2'
    save
...
def _paga_graph(
    ...
    # draw directed edges
        ...
        if min_edge_width is not None or max_edge_width is not None:
            widths = np.clip(widths, min_edge_width, max_edge_width)
        nx.draw_networkx_edges(
            g_dir, pos, ax=ax, width=widths, edge_color='black', arrowsize=arrowsize, connectionstyle=connectionstyle
        )
