...
# %%
filt = ((adata.obs.DCfate < 0.38) & (adata.obs.Neufate>0.24) & ~adata.obs.leiden.isin(['12', '25', '16']))
adata.obs['filt'] = filt.astype(str)

# %%
sc.pl.scatter(adata,basis='umap_2d',color=['filt'],
              size = 30,palette=['k','y'],save='_select_cells_clu_10',title = 'Neu')
...
sc.pl.scatter(adata_rid,basis = 'umap_2d',color = ['leiden'],
              legend_loc='on data',size=  30,title = '',save='_selected_cells_clu_10')

# %%
sc.pl.scatter(adata_rid,basis = 'umap_2d',color = ['dpt_pseudotime'],
              legend_loc='on data',save='_dpt_clu_10',size = 30)
...
sc.pl.scatter(adata_rid,basis = 'umap_2d',color = ['dpt_pseudotime_scaled'],
              legend_loc='on data',size=  30,save='_scaled_dpt_clu_10')
...
# %%
df.to_csv('./tables/input_pseudo_dyn_clu_10_dpt.csv',index=None)
...
# %%
df2.columns = df2.columns.add_categories(['traj_neu'])

# %%
df2['traj_neu'] = list(np.sum(df2,axis=1)/df1['sc_ncells_total']*df1['flow_total'])
...
for j in np.unique(df1.time):
    ...
    mean[k] = np.mean(df2.loc[df1.time == j].traj_neu)
    std[k] = np.std(df2.loc[df1.time == j].traj_neu)/np.sqrt(np.sum(df1.time == j))
...
# %%
df.to_csv('./tables/input_pseudo_dyn_clu_10_size.csv',index=None)
